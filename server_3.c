...
void createTable()
{
	...
	for (i = 0; i < 3; i++) // initialisation du tableau a 0
		for (j = 0; j < 8; j++)
			tableCartes[i][j] = 0;

	for (i = 0; i < 3; i++) // nombre des joueurs
	{
		for (j = 0; j < 4; j++) // nombre des cartes par joueur
		{
			c = deck[i * 4 + j]; // l'es[ace pour chaque 4 cartes d'un joueur
			switch (c)
			{
			...
			case 'C':
				...
				if (nbClients == 3)
				{
					...
					for (int local_clients = 0; local_clients < nbClients; local_clients++)
					{
						...
						sprintf(send_buffer, "D %d %d %d %d", deck[index], deck[index + 1], deck[index + 2], deck[index + 3]); // we are sending 4 cards
						...
						index += 4;		   // goes up to next client index in the list
