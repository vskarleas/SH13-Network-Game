...
volatile int synchro; // synchro=1 : le thread serveur tcp a reçu un message
					  // synchro=0 : le thread serveur tcp n'a pas reçu de message

// Volatile memory: qu'on ne peut pas se fier sur la valeur de synchro quand on fait un read dessous. We do not want to check the processor's cache, but we want to lookup the real number of synchro that is updated on two different threads [on the resseau side - tcp server] and [graphics side]
...
void *fn_serveur_tcp(void *arg)
{
	...
	listen(sockfd, 5); // 5 here means accumulation of 5 connections => 1 connection per second ... 5 connections per 5 seconds ??
	...
	while (1)
	{
		...
		bzero(gbuffer, 256); // gbuffer = global buffer
...
int main(int argc, char **argv)
{
	...
	while (!quit)
	{
		if (SDL_PollEvent(&event)) // recuperer les events. Dans la boucle raphique, faut integer la boucle resseau
		{
			// printf("un event\n");
			switch (event.type) /* Redesign part */
			{
			case SDL_QUIT:
				quit = 1;
				break;
			case SDL_MOUSEBUTTONDOWN:		 // If we choose with mouse button
				...
				else if ((mx >= 0) && (mx < 200) && (my >= 90) && (my < 270)) // If we click on a line of the grid (a player from the list)
				{
		...
		SDL_RenderDrawLine(renderer, 320, 0, 320, 270);
		SDL_RenderDrawLine(renderer, 380, 0, 380, 270);
		SDL_RenderDrawLine(renderer, 440, 0, 440, 270);
		SDL_RenderDrawLine(renderer, 500, 0, 500, 270);
		SDL_RenderDrawLine(renderer, 560, 0, 560, 270);
		SDL_RenderDrawLine(renderer, 620, 0, 620, 270);
		SDL_RenderDrawLine(renderer, 680, 0, 680, 270);
